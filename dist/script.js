  ...
  function createGrids(res) {
    for (var i = 0; i < res.gridMass.length; i++) {
      for (var j = 0; j < res.gridMass[i].length; j++) {
        ...
        var color = "green";
        var storage_image = "Storage.jpg";var exit_image = "Fura.jpg";
...
var Game = function () {
  function Game(data) {
    ...
    createField({ gridMass: this.gridMass, size: this.size, storageValue: this.storageValue, maxChestCarry: this.maxChestCarry });
    //создаём игрока и рисуем его на выходе
    this.player = new Player({ size: this.size, maxChestCarry: this.maxChestCarry });
    this.player.drawPlayer({ xPPosition: this.gridMass[this.numberXGrid - 1][this.numberYGrid - 1].xPosition,
        ...
        if (code === "Space") {
          if (this.gridMass[this.personPosition.xPos][this.personPosition.yPos].isStorage == 0) {
            ...
            if (this.player.isCanDrop()) {
              ...
              this.storageValue[0]++;
              chestOnHandsShow(this.player.chestCarry, this.storageValue[0], { x: this.personPosition.xPos, y: this.personPosition.yPos });
            }
          }
          if (this.gridMass[this.personPosition.xPos][this.personPosition.yPos].isStorage > 0) {
...
function dataCreator(data) {
  ...
  var gridHeight = height / data[1];
  var gridWidth = width / data[0];
  var storageMass = [];
  var set = new Set();
  ...
  console.log("array of storages");
  console.log(storageMass);
  var storageValue = [];

  storageValue[0] = 0; //на выходе пусто
  for (var i = 1; i <= data[2]; i++) {
    var number = getRandomInRange(1, data[3]);
    storageValue[i] = number;
  }
  console.log(storageValue);
  GameEx = new Game({ storageMass: storageMass, storageValue: storageValue, x: data[0], y: data[1], personNumber: data[4] });
}
